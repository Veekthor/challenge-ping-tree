...
function validateVisitor (data, cb) {
  if (!data.geoState || !data.timestamp) {
    return cb(new Error('geoState and timestamp are required'), null)
  }
  if (typeof data.geoState !== 'string' || data.geoState.length !== 2) {
    return cb(new Error('geoState must be a 2 character string'))
  }
  var timestamp = new Date(data.timestamp)
  if (!(timestamp instanceof Date && !isNaN(timestamp))) {
    return cb(new Error('timestamp must be a valid date'))
  }
